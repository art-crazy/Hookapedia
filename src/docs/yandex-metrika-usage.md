...
## Отключение в локальной разработке

Создайте файл `.env.local` и добавьте переменную `NEXT_PUBLIC_YM_ENABLED=false`. При таком значении компонент метрики не будет подключаться и события из хуков отправляться не будут.
